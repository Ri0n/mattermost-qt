...
class ChannelPostSource : public AbstractPostSource {
  Q_OBJECT
public:
  explicit ChannelPostSource(Backend &backend, BackendChannel &channel,
  ...
  void requestRange(int first, int last, RequestReason reason,
...
private:
  ...
  int findFreeWindowFirst(const QVector<QString> &ids, int windowSize,
