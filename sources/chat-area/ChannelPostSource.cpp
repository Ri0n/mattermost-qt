...
Q_LOGGING_CATEGORY(lcTimelineChannel, "mattermost.timeline.channel",
                   QtWarningMsg)
...
QVector<BackendPost *> cachedRootPosts(const BackendChannel &channel) {
  ...
  std::sort(result.begin(), result.end(),
            [](const BackendPost *lhs, const BackendPost *rhs) {
              if (!lhs || !rhs) {
                return lhs != nullptr;
              }
...
ChannelPostSource::ChannelPostSource(Backend &backendInstance,
                                     ...
                                     QObject *parent)
    : AbstractPostSource(parent), backend(backendInstance),
  ...
  if (exactRootCount) {
    ...
    QTimer::singleShot(
        0, this, [this] { requestBeforeFirst(RequestReason::Initial, 0); });
  }
  ...
  connect(&channel, &BackendChannel::onPostDeleted, this,
          [this](const QString &postId) {
            ...
            BackendPost *deletedPost =
                channel.postIdToPost.value(postId, nullptr);
            const bool removeLikeOfficialClient =
                deletedPost && deletedPost->isOwnPost() &&
                deletedPost->root_id.isEmpty() && !deletedPost->hidden;
...
bool ChannelPostSource::adoptNavigationContext(
    const QString &targetPostId, const QStringList &chronologicalIds,
    bool reachedOldest, bool reachedNewest) {
  ...
  if (reachedOldest && !acceptFullExactFirst(0)) {
    qCWarning(lcTimelineChannel)
        << "conflicting oldest full navigation context" << targetPostId;
    return false;
  }
  if (reachedNewest && !acceptFullExactFirst(logicalCount - validCount)) {
    qCWarning(lcTimelineChannel)
        << "conflicting newest full navigation context" << targetPostId;
    return false;
  }
  for (int offset = 0; offset < validCount; ++offset) {
    ...
    if (!acceptFullExactFirst(existing - offset)) {
      qCWarning(lcTimelineChannel)
          << "inconsistent full authoritative context overlap" << targetPostId
          << id << existing << offset;
      return false;
    }
  }
  ...
  const QStringList selected =
      validIds.mid(selectedFirst, selectedLast - selectedFirst + 1);
  const bool selectedReachedOldest = reachedOldest && selectedFirst == 0;
  const bool selectedReachedNewest =
      reachedNewest && selectedLast == validCount - 1;
  const int exactSelectedFirst =
      fullExact ? fullExactFirst + selectedFirst : -1;
  return placeNavigationContext(targetPostId, selected, selectedReachedOldest,
                                selectedReachedNewest, exactSelectedFirst);
}

void ChannelPostSource::requestRange(int first, int last, RequestReason reason,
                                     quint64 generation) {
  Q_UNUSED(reason)
  Q_UNUSED(generation)
  ...
  const int requestedLast =
      std::min(static_cast<int>(postIds.size()) - 1, last);
...
void ChannelPostSource::resolveOldestBoundary(
    int emptyPage, std::function<void()> completion) {
  if (completion) {
...
void ChannelPostSource::probeOldestBoundary() {
  ...
  if (oldestBoundaryNonEmptyPage < 0) {
    ...
    oldestBoundaryProbeStep =
        std::min(oldestBoundaryEmptyPage + 1, oldestBoundaryProbeStep * 2);
  } else {
  ...
  qCDebug(lcTimelineChannel).nospace()
      << "OLDEST_BOUNDARY_PROBE page=" << page << " offset=" << offset
      << " nonEmpty=" << oldestBoundaryNonEmptyPage
      << " empty=" << oldestBoundaryEmptyPage
      << " step=" << oldestBoundaryProbeStep << " perPage=1";
  ...
  PostTimelineService::instance(backend).loadChannelPage(
      ...
      [guard, page, offset](const PostTimelineService::Page &result) {
        ...
        qCDebug(lcTimelineChannel).nospace()
            << "OLDEST_BOUNDARY_RESULT page=" << page << " offset=" << offset
            << " exists=" << exists;

        if (exists) {
          guard->oldestBoundaryNonEmptyPage =
              std::max(guard->oldestBoundaryNonEmptyPage, page);
        } else {
          guard->oldestBoundaryEmptyPage =
              std::min(guard->oldestBoundaryEmptyPage, page);
        }
...
void ChannelPostSource::loadOldestBoundaryPage(int page) {
  ...
  qCDebug(lcTimelineChannel).nospace()
      << "OLDEST_BOUNDARY_PAGE page=" << page << " perPage=" << ServerPageSize;
...
void ChannelPostSource::reconcileRootCount(int actualCount) {
  ...
  qCDebug(lcTimelineChannel).nospace()
      ...
      << " oldCount=" << postIds.size() << " actualCount=" << actualCount;
...
void ChannelPostSource::requestBeforeFirst(RequestReason reason,
                                           quint64 generation) {
  Q_UNUSED(reason)
  Q_UNUSED(generation)
  ...
  auto completed = [guard](const PostTimelineService::Page &result) {
    ...
    guard->moreBeforeFirst =
        !result.prevPostId.isEmpty() && !discovered.isEmpty();
  };
...
int ChannelPostSource::currentLogicalCount() const {
  if (exactRootCount) {
    const int correctedServerCount =
        std::max(0, channel.total_msg_count_root - rootCountOverestimate);
...
int ChannelPostSource::initialBoundaryProbePages() const {
  const qint64 numerator =
      static_cast<qint64>(postIds.size()) * InitialBoundaryProbePercent;
  const qint64 denominator = 100LL * ServerPageSize;
  return std::max(
      1, static_cast<int>((numerator + denominator - 1) / denominator));
}
...
int ChannelPostSource::estimateIndexForPost(const BackendPost &post) const {
  ...
  for (int index = 0; index < count; ++index) {
    ...
    if (current->create_at <= post.create_at &&
        current->create_at >= lowerTime) {
      lowerTime = current->create_at;
      lowerIndex = index;
    }
    if (current->create_at >= post.create_at &&
        current->create_at <= upperTime) {
      upperTime = current->create_at;
      upperIndex = index;
    }
  }

  const auto interpolate = [](int firstIndex, uint64_t firstTime, int lastIndex,
                              uint64_t lastTime, uint64_t targetTime) {
    if (lastIndex <= firstIndex || lastTime <= firstTime) {
      return firstIndex;
    }
    const long double fraction = std::max<long double>(
        0.0L, std::min<long double>(
                  1.0L, static_cast<long double>(targetTime - firstTime) /
  ...
  if (lowerIndex >= 0 && upperIndex >= 0) {
    ...
    if (upperIndex > lowerIndex && upperTime > lowerTime) {
      return interpolate(lowerIndex, lowerTime, upperIndex, upperTime,
                         post.create_at);
    }
  }
  ...
  if (lowerIndex >= 0) {
    ...
    if (lowerIndex < count - 1 && newestTime > lowerTime &&
      ...
      return interpolate(lowerIndex, lowerTime, count - 1, newestTime,
                         post.create_at);
    }
    return lowerIndex;
  }
...
bool ChannelPostSource::placeNavigationContext(
    const QString &targetPostId, const QStringList &chronologicalIds,
    bool reachedOldest, bool reachedNewest, int exactFirstHint) {
  ...
  const int oldTargetIndex =
      oldWindow.isValid() && oldWindow.targetPostId == targetPostId
          ? indexOfPost(targetPostId)
          : -1;
  ...
  if (reachedOldest && !acceptExactFirst(0)) {
    qCWarning(lcTimelineChannel)
        << "conflicting oldest navigation context" << targetPostId;
    return false;
  }
  if (reachedNewest && !acceptExactFirst(count - contextCount)) {
    qCWarning(lcTimelineChannel)
        << "conflicting newest navigation context" << targetPostId;
    return false;
  }
  ...
  for (int offset = 0; offset < contextCount; ++offset) {
    ...
    if (!acceptExactFirst(existing - offset)) {
      qCWarning(lcTimelineChannel)
          << "inconsistent authoritative context overlap" << targetPostId << id
          << existing << offset;
      return false;
    }
  }
  ...
  if (exactPlacement) {
    ...
    for (int offset = 0; offset < contextCount; ++offset) {
      ...
      if (!existing.isEmpty() && existing != ids.at(offset)) {
        qCWarning(lcTimelineChannel)
            << "authoritative navigation context collision" << targetPostId
            << first + offset << existing << ids.at(offset);
        return false;
      }
    }
  } else {
    const int preferredTarget =
        oldTargetIndex >= 0 ? oldTargetIndex : estimateIndexForPost(*target);
    const int preferredFirst = preferredTarget - targetOffset;
    first = findFreeWindowFirst(next, contextCount, preferredFirst);
    if (first < 0) {
      qCWarning(lcTimelineChannel)
          << "no free provisional context span" << targetPostId
          << preferredFirst << contextCount;
      return false;
    }
  }
  ...
  qCDebug(lcTimelineChannel).nospace()
      << "NAV_CONTEXT target=" << targetPostId << " first=" << first
      << " last=" << newLast << " targetIndex=" << first + targetOffset
      << " exact=" << exactPlacement << " reachedOldest=" << reachedOldest
...
void ChannelPostSource::seedCachedPosts() {
  ...
  if (postIds.size() > cached.size() && channel.last_post_at != 0 &&
  ...
  const int first =
      std::max(0, static_cast<int>(postIds.size() - cached.size()));
...
void ChannelPostSource::removeLogicalRange(int first, int count) {
  ...
  count =
      std::max(0, std::min(count, static_cast<int>(postIds.size()) - first));
  if (count == 0) {
    return;
  }
  ...
  if (provisionalWindow.isValid()) {
    ...
    const int removedBeforeWindow =
        first < oldFirst ? std::min(count, oldFirst - first) : 0;
    ...
    if (overlapFirst <= overlapLast) {
      ...
      const QStringList removedIds =
          provisionalWindow.postIds.mid(offset, overlapCount);
      const bool targetRemoved =
          removedIds.contains(provisionalWindow.targetPostId);
...
void ChannelPostSource::placePage(int page,
  ...
  if (chronologicalIds.size() < ServerPageSize) {
    reconcileRootCount(page * ServerPageSize +
                       static_cast<int>(chronologicalIds.size()));
  }

  const int first =
      std::max(0, static_cast<int>(postIds.size()) - page * ServerPageSize -
                      static_cast<int>(chronologicalIds.size()));
...
void ChannelPostSource::prependDiscovered(const QStringList &chronologicalIds) {
  ...
  combined.reserve(static_cast<int>(chronologicalIds.size()) +
                   static_cast<int>(postIds.size()));
  for (const QString &id : chronologicalIds) {
...
void ChannelPostSource::appendLivePost(BackendPost &post) {
  ...
  int count = exactRootCount ? currentLogicalCount()
